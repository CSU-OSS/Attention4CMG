...
def get_change_tokens(source_str):
    ...
    added_tokens = []
    deleted_tokens = []
    modified_tokens = []  # List of (del_tokens, add_tokens)
    context_tokens = []
    source_tokens = []

    i = 0
    in_context_block = False  # 标记是否处于 CONTEXT 区块中
    ...
    while i < len(lines):
        ...
        if line.startswith('-'):
            ...
            if add_lines:
                ...
                source_tokens.extend(["<MOD-OLD>"] + del_tokens)
                source_tokens.extend(["<MOD-NEW>"] + add_tokens + ["</MOD-END>"])
            else:
                for line in del_lines:
                    toks = tokenize_line(line)
                    deleted_tokens.extend(toks)
                    source_tokens.extend(["<DELETED>"] + toks + ["</DELETED>"])

        elif line.startswith('+'):
            toks = tokenize_line(line.strip())
            added_tokens.extend(toks)
            source_tokens.extend(["<ADDED>"] + toks + ["</ADDED>"])
            i += 1
...
def generate_masks(source_tokens, tokenizer, window_size=64):
    ...
    for token in source_tokens:
        if token == "<ADDED>":
            ...
            change_mask.append(False)  # 填充位置标记为False
        else:
    ...
    # 处理特殊token（只在有效序列范围内操作）
    special_tokens = ["<ADDED>", "</ADDED>", "<DELETED>", "</DELETED>", "<CONTEXT>", "</CONTEXT>",
                     "<MOD-OLD>", "<MOD-NEW>", "<MOD-END>",
