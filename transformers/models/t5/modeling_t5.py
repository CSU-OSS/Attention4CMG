...
print("Using T5 model from transformers.models.t5.modeling_t5.py")
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.change_bias = nn.Parameter(torch.tensor(3.0))
    ...
    def forward(
        ...
        if local_mask is not None:
            # 扩展维度：(batch_size, seq_length, key_length) → (batch_size, 1, seq_length, key_length)
            local_mask = local_mask.unsqueeze(1)
            # 屏蔽局部范围外的位置
            scores = scores.masked_fill(local_mask == 0, -1e9)

        # 处理 change_mask（增强变更位置的注意力）
        if change_mask is not None:
            # 扩展维度：(batch_size, key_length) → (batch_size, 1, 1, key_length)
            change_mask = change_mask.unsqueeze(1).unsqueeze(2)
            # 增强变更位置的分数
            scores = scores + (change_mask * self.change_bias)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if ra_mask is not None and labels is not None:
            ...
            adjusted_logits = lm_logits * ra_mask.unsqueeze(-1)  # Ensure ra_mask is broadcastable
