...
def convert_examples_to_features(item):
    ...
    ra_mask = create_ra_mask(example.target, tokenizer)
    if len(ra_mask) > args.max_target_length:
        ra_mask = ra_mask[:args.max_target_length]
    else:
        ra_mask.extend([0] * (args.max_target_length - len(ra_mask)))
...
def create_ra_mask(text, tokenizer):
    # Process the single text with spaCy
    doc = nlp(text)
    word_list = [token.text for token in doc]

    # Tokenize the single text
    encoding = tokenizer(text=[word_list], is_split_into_words=True, padding=True, truncation=True, max_length=100)

    # Initialize ra_mask to 0 for all tokens
    ra_mask = [0] * len(encoding['attention_mask'][0])

    word_ids = encoding.word_ids(batch_index=0)  # Get word_ids for the single text
        ...
        if token.pos_ == 'VERB':
            ...
            for child in token.children:
                if child.dep_ in ['dobj', 'iobj', 'pobj', 'nsubjpass']:
                    ...
                    # Set ra_mask to 1 for verb-object tokens
...
def post_process_feature(args_tuple):
    try:
        ...
        return InputFeatures(
            ...
            change_mask=change_mask,
            url=example.url,
            ra_mask=feature.ra_mask
        )
    except Exception as e:
        print(f"[ERROR] 子进程异常: {e}")
        import traceback
        traceback.print_exc()
        return None  # 或 raise 让 Pool 报错
